...
void healingmove::heal_health_player(playable* pptr){
    ...
    std::cout << "Recovered " << health_recovered << " health for " << pptr->get_Name() << std::endl;
...
void healingmove::heal_health_chad(Gigachad* gigaptr){
    ...
    if ((health_recovered + gigaptr->get_Current_HP() )> gigaptr->get_Max_HP()){ 
        health_recovered = gigaptr->get_Max_HP() - gigaptr->get_Current_HP();
    } 

    std::cout << "Recovered " << health_recovered << " health for " << gigaptr->get_Name() << std::endl;
